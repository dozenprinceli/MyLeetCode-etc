...
class Solution {
    ...
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        final Deque<int[]> stk = new LinkedList<>();
        for (String log : logs) {
            int[] curLog = parseStr(log);
            // 新函数入栈
            if (curLog[1] == START) {
                if (stk.isEmpty()) {
                    stk.push(new int[]{curLog[0], curLog[2]});
                } else {
                    int oldFunc = stk.peek()[0];
                    int oldTime = stk.peek()[1];
                    int newTime = curLog[2];
                    stk.push(new int[]{curLog[0], newTime});
                    times[oldFunc] += newTime - oldTime;
                }
            }
            // 顶函数出栈, 并更新上一个函数时间戳
            else {
    ...
    private int[] parseStr(String log) {
        String[] info = log.split(":");
        // result数组存储信息, 格式为{函数标识符, 开始/结束, 时间}
        int[] result = new int[3];
        result[0] = parseInt(info[0]);
        result[1] = info[1].equals("end") ? END : START;
        // 所有时间均为时间戳的起始时刻
        result[2] = result[1] == END ? parseInt(info[2]) + 1 : parseInt(info[2]);
        return result;
    }
    ...
    private int parseInt(String str) {
        ...
        for (int i = 0; i < str.length(); ++i) {
            int cur = ((int) str.charAt(str.length() - i - 1)) - 48;
